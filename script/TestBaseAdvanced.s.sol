...
contract TestBaseAdvanced is Script {
    ...
    function phase18_BufferEviction() internal {
        ...
        for (uint i = 0; i < 400; i++) {
            ...
            vm.warp(block.timestamp + 2 hours + 1); // Sell lock
            
            // Add LP (0.5 ETH each -> all equal contributions)
            vm.startBroadcast(lpUser);
            uint256 balance = token.balanceOf(lpUser);
        ...
        if (snapshotTaken) {
            ...
            vm.startBroadcast(largeLP); // Participant can finalize
    ...
    function phase19_LPRewardFull() internal {
        ...
        if (snapshot && participants >= 50) {
            ...
            (address[] memory top100, uint256[] memory contributions,) = lpVault.getLeaderboard(100);
            ...
            console.log("\nRANKS 11-100 (40% of pool):");

				uint256 totalSecondaryRewards = 0;
				for (uint i = 10; i < 100 && i < top100.length; i++) {
                uint256 reward = lpVault.roundRewards(0, top100[i]);
            ...
            console.log("    Ranks 11-100 expected (40%):", expectedSecondary / 1e18, "ETH");
			console.log("    Ranks 11-100 actual:", totalSecondaryRewards / 1e18, "ETH");
            ...
            // Ranks 11-50: All contributed 5 ETH -> should get equal rewards
            uint256 reward10 = lpVault.roundRewards(0, top100[10]);
            uint256 reward49 = lpVault.roundRewards(0, top100[49]);
            console.log("\n    User #10 (5 ETH):", reward10 / 1e15, "finney");
            console.log("    User #49 (5 ETH):", reward49 / 1e15, "finney");
            console.log("    Ratio:", (reward10 * 100) / reward49, "/ 100 (expect ~100)");
            
            // Cross-tier: User #0 (10 ETH, 75% pool) vs User #10 (5 ETH, 25% pool)
            // User 0: (10 / 100 ETH total top10) * 60% pool = 6% of total
			// User 10: (5 / 200 ETH total ranks11-100) * 40% pool = 1% of total
			// Ratio: 6 / 1 = 6x
            console.log("\n    Cross-tier:");
